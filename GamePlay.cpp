...
static Card card[10] = { 0 }; /*ÀÏ½ÃÀûÀ¸·Î »ç¿ëÇÒ »Ó ³ªÁß¿¡´Â Çì´õÆÄÀÏ¿¡¼­ °¡Á®¿À´Â °É·Î ÇÕ´Ï´Ù.*/
...
void DisplayGame(HWND hWnd, HDC hDC, Player* player)
{
	...
	/*Ä«µå ¼³Á¤(ÇöÀç´Â ¾Æ¹«°Íµµ ¾øÀ½)*/
	WCHAR str[20];
	wsprintf(str, L"ÄÚ½ºÆ® : %d", player->cost);
	TextOut(hDC, 100, 400, str, lstrlen(str));

	if (isMyTurnPrint)
	{
		TextOut(hDC, 550, 100, L"´ç½ÅÀÇ Â÷·ÊÀÔ´Ï´Ù.", lstrlen(L"´ç½ÅÀÇ Â÷·ÊÀÔ´Ï´Ù."));
	}

	if (isEnemyTurnPrint)
	{
		TextOut(hDC, 550, 100, L"ÀûÀÇ Â÷·ÊÀÔ´Ï´Ù.", lstrlen(L"ÀûÀÇ Â÷·ÊÀÔ´Ï´Ù.")); 
	}
...
void SetImg()
{
	...
	{
		AttackCardImg.Load(L"°ø°ÝÄ«µå.png");
	}
	if (DeffenceCardImg.IsNull())
	{
		DeffenceCardImg.Load(L"¹æ¾îÄ«µå.png");
	}
	if (turnEnd.IsNull())
	{
